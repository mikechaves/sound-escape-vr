...
public class PlaybackManager : MonoBehaviour {
   ...
   void Start() {
      ...
      numChannels = TileManager.Instance.nChannels;
        //numChannels2 = TileManager2.Instance.nChannels;
   ...
   public void Update() {
      ...
      // figure out which notes to play
      ...
      {
          if (!TileManager2.Instance.Tiles[j, computedSlot].PlayState)
              continue;

          // play the note and show the visual
          SampleManager2.Instance.StopSample(j);
          SampleManager2.Instance.PlaySample(j, nextBeatTime);
          TileManager2.Instance.Tiles[j, computedSlot].OnPlayHit();
      }

      for (int k = 0; k < numChannels; k++)
      {
          if (!TileManager3.Instance.Tiles[k, computedSlot].PlayState)
              continue;

          // play the note and show the visual
          SampleManager3.Instance.StopSample(k);
          SampleManager3.Instance.PlaySample(k, nextBeatTime);
          TileManager3.Instance.Tiles[k, computedSlot].OnPlayHit();
      }
