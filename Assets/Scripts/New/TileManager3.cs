...
public class TileManager3 : MonoBehaviour {
   ...
   public static TileManager3 Instance { get; set; }
   #endregion
   ...
   void Awake() {
        if (TileManager3.Instance == null)
            TileManager3.Instance = this;
   }

   // Use this for initialization
   void Start() {
        // TODO: initialize the grid
        nChannels = SampleManager2.Instance.numChannels;
      ...
      tileStates = new bool[nChannels, nSlots];
        ...
        //float parentWidth = gameObject.GetComponent<Transform>().position.x;
        float parentHeight = gameObject.GetComponent<RectTransform>().rect.height;
        //float parentHeight = gameObject.GetComponent<Transform>().position.y;
